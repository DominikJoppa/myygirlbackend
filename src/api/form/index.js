...
router.post('/models', async (req, res) => {
  const { social, username, email } = req.body;
  ...
  try {
    const { data: social_data } = await supabase
      .form('creators_form')
      .insert({ type: social, creators_id: username, email });
...
router.post('/category', async (req, res) => {
  const { category, username, email } = req.body;
  ...
  try {
    const { data: categoryData } = await supabase
      .form('creators_form')
      .insert({ type: category, creators_id: username, email });
    const dataJSON = {
      status: 'success',
      message: 'Your suggestion has been added and is awaiting verification.',
      data: categoryData,
